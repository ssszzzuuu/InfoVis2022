class BarChart{
    ...
    update() {
        ...
        //データの追加
        if (num == 1){
            self.bvalue = d => d.Member	;
        }else if (num == 2){
            self.bvalue = d => d.Population	;
        }else if(num == 3){
            self.bvalue = d => d.PopulationPerMember	;
        }else if(num == 4){
            self.bvalue = d => d.DisparityPerMember	;
        }else if(num == 5){
            self.bvalue = d => d.Voter	;
        }else if(num == 6){
            self.bvalue = d => d.VoterPerMember	;
        }else if(num == 7){
            self.bvalue = d => d.DisparityPerVoter	;
        }else if(num == 8){
            self.bvalue = d => d.AreaPerMember	;
        }else if(num == 9){
            self.bvalue = d => d.DisparityPerArea	;
        }else{
            self.bvalue = d => d.Population	;
        }
        ...
        self.yscale.domain(self.data.map(d => d.Prefecture)).paddingInner(0.1);
        ...
        self.chart.selectAll("rect")
            .on('mouseover', (e,d) => {
                d3.select('#tooltip')
                .style('opacity', 1)
                .html(`<div class="tooltip-label">${d.Prefecture}</div>(${self.bvalue(d)})`);
            })
            .on('mousemove', (e) => {
                const padding = 10;
                d3.select('#tooltip')
