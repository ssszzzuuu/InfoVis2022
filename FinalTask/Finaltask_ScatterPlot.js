class ScatterPlot {
    ...
    update() {
        ...
        //データの追加
        if (Horizontalnum == 1){
            self.xvalue = d => d.Member	;
        }else if (Horizontalnum == 2){
            self.xvalue = d => d.Population	;
        }else if(Horizontalnum == 3){
            self.xvalue = d => d.PopulationPerMember    ;
        }else if(Horizontalnum == 4){
            self.xvalue = d => d.DisparityPerMember	;
        }else if(Horizontalnum == 5){
            self.xvalue = d => d.Voter	;
        }else if(Horizontalnum == 6){
            self.xvalue = d => d.VoterPerMember	;
        }else if(Horizontalnum == 7){
            self.xvalue = d => d.DisparityPerVoter	;
        }else if(Horizontalnum == 8){
            self.xvalue = d => d.AreaPerMember	;
        }else if(Horizontalnum == 9){
            self.xvalue = d => d.DisparityPerArea	;
        }else{
            self.xvalue = d => d.Population	;
        }
        ...
        //データの追加
        if (Verticalnum == 1){
            self.yvalue = d => d.Member	;
        }else if (Verticalnum == 2){
            self.yvalue = d => d.Population	;
        }else if(Verticalnum == 3){
            self.yvalue = d => d.PopulationPerMember	;
        }else if(Verticalnum == 4){
            self.yvalue = d => d.DisparityPerMember	;
        }else if(Verticalnum == 5){
            self.yvalue = d => d.Voter	;
        }else if(Verticalnum == 6){
            self.yvalue = d => d.VoterPerMember	;
        }else if(Verticalnum == 7){
            self.yvalue = d => d.DisparityPerVoter	;
        }else if(Verticalnum == 8){
            self.yvalue = d => d.AreaPerMember	;
        }else if(Verticalnum == 9){
            self.yvalue = d => d.DisparityPerArea	;
        }else{
        self.yvalue = d => d.Population	;
        }
    ...
    render() {
        ...
        circles
            .on('mouseover', (e,d) => {
                d3.select('#tooltip')
                .style('opacity', 1)
                .html(`<div class="tooltip-label">${d.Prefecture}</div>(${self.xvalue(d)}, ${self.yvalue(d)})`);
            })
            .on('mousemove', (e) => {
                const padding = 10;
                d3.select('#tooltip')
